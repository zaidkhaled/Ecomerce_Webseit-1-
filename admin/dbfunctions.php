...
<?php } else {
    ...
    //      <============= start items page ================>  
     ...
     // start item page
      ...
      $commentsID = isset($_GET['ajxID']) && is_numeric($_GET['ajxID'])? intval($_GET['ajxID']) :0;
    ...
    // make an edit on comment 
            ...
            $commentID = isset($_GET['ajxID']) && is_numeric($_GET['ajxID'])? intval($_GET['ajxID']) :0;
    ...
    if(preg_match('/members/', $fromPage)) { 
         ...
         $query = $_GET['ajxdata_required'] == 'pending'? "AND regStatus = 0" : " "; 
                   ...
                   <li class="collection-item">
                       <div><?php echo $item['Name'] ?>
                         ...
                         <?php if ($item['approve'] == 0 or $item['approve'] == null){ ?>
                                    ...
                                    <a data-id="<?php echo $item['Item_ID'] ?>" id='unapproved' class='secondary-content'>
                                      <i class='material-icons'>thumb_down</i>
                                    </a>
                            <?php }?>
                      </div>
...
<!--              Note <=========> in "sure Msg" <to delete user> from jave script href will equal #modal + userID  
           ...
           <?php }else { ?>
                 ...
                 <?php }?>
                 <!--Edit Butten-->
                 <a class='btn-floating teal update-item-btn'>
                   <i class='large material-icons'>mode_edit</i>
                 </a><!-- end Edit Butten-->
                 <!--Delete Butten-->
       ...
       //chick if http_referer contains comments, if yes then show this list just inside it
        ...
        if (!empty($_GET['ajxdata_required'])){
            
        $data_require = $_GET['ajxdata_required'];
...
?>
           ...
           <?php if ($row['Status'] == null) { ?>
                 <!--if item is not activated then show unactivated button-->
                 <a class='btn-floating pink darken-4' id='unapproved' data-id ='<?php echo $row['C_ID']; ?>'>
                   <i class="material-icons">thumb_down</i>
                 </a>
...
<!--                  else if user is activated then show OK button-->
                 ...
                 <!--Delete Butten-->
                 <a class='btn-floating red modal-trigger' data-id="<?php echo $row['C_ID'];?>"  href="#modal<?php echo $row['C_ID'];?>">
                   <i class='large material-icons '>delete_forever</i>
                 </a> <!--Delete Butten-->
                 ...
                 <div id="modal<?php echo $row['C_ID']; ?>" class='modal' >
                   ...
                   <div class='modal-footer'>
                     <a class='modal-action modal-close waves-effect waves-green btn-flat'>Close</a>    
                     <a class='modal-action delete modal-close waves-effect waves-green btn-flat' data-id='<?php echo $row['C_ID']; ?>'>Delete</a>
                  </div> <!--end modal Delete Butten -->
                </div>
              </td>
            </tr>
       ...
       //chick if http_referer contains dashpoard, if yes then show this list just inside it
                ...
                $itemID = isset($_GET['ajxItem_ID']) && is_numeric($_GET['ajxItem_ID'])? intval($_GET['ajxItem_ID']) : 0;
                    ...
                    <?php foreach($rows as $row) {?>
                            ...
                            <li class="collection-item  avatar" data-id='<?php echo $row['C_ID']; ?>' item-id='<?php echo $row['Item_ID']; ?>'>
                               <div class="comment-controll">
                                 <a  class="secondary-content update-btn-to-item">
                                   <i class="material-icons ">border_color</i>
                                 </a>
                                 <a class="secondary-content delete-btn-to-item modal-trigger " data-id="<?php echo $row['C_ID'];?>"  href="#modal-item<?php echo $row['C_ID'];?>" >
                               ...
                               <!--start modal-delete comment-->
                                 <div id="modal-item<?php echo $row['C_ID']; ?>" class='modal' >
                                   ...
                                   <div class='modal-footer'>
                                     <a class='modal-action modal-close waves-effect waves-green btn-flat'>Close</a>    
                                     <a class='modal-action delete-to-item modal-close waves-effect waves-green btn-flat' id= "delete-to-item" data-id='<?php echo $row['C_ID']; ?>' item-id='<?php echo $row['Item_ID']; ?>'>Delete</a>
                                  </div> <!--end modal Delete Butten -->
                                </div>    
